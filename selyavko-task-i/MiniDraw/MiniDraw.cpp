...
{
	CBrush* brush;
	brush = new CBrush(this->color);
	PDC->SelectObject(brush);

	CPen aPen;
	aPen.CreatePen(PS_SOLID, 2, this->color);
...
{
	if (ar.IsStoring())
		ar << m_X1 << m_Y1 << m_X2 << m_Y2 << color << type;
	else
		ar >> m_X1 >> m_Y1 >> m_X2 >> m_Y2 >> color >> type;
}
...
void CMiniDrawDoc::AddLine(int X1, int Y1, int X2, int Y2, COLORREF color, bool figureType)
{
	if (top != GetNumLines())
	{
		while (top != GetNumLines())
		{
			int Index = (int)m_LineArray.GetSize() - 1;
	...
	CLine* pLine = new CLine(X1, Y1, X2, Y2, color, figureType);
